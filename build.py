...
def run_plastex(args, target):
    print(f"[*] Running plasTeX with target {target} and the following args:")
    plastex_args = " "
    for arg in args:
        print(f"    {arg}")
        plastex_args += arg + " "
...
def posts(root_file, gen_dir):
    ...
    print("[*] Copied posts-commgroup.css")
    ...
    print("[*] Found template start and end")

    # Render each post
    posts = next(os.walk("posts/"))[1]
    for post in posts:
        print(f"[*] Building post {post}")
        ...
        print(f"[*] Built fragment for {post}")
        ...
        print(f"[*] Wrote {post} to full file")
...
def fix_index(gen_dir):
    ...
    print("[*] Fixed title of index.html")


def build_main(root_file, gen_dir):
    print("[*] Building main files")
